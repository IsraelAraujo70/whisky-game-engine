...
// keyBinding associates an X11 keycode with an engine control name.
type keyBinding struct {
	keycode byte
	control string
}

// nameToKeysym maps human-readable key names used by whisky.Config.KeyMap to
// X11 keysyms. The chosen values match the common X11 definitions.
var nameToKeysym = map[string]uintptr{
	// Letters
...
func buildKeyBindings(keyMap map[string]string, lookup func(keysym uintptr) byte) []keyBinding {
